...
function syncronize_group(data, inds, group){
    try{
        if (! group.hasUser(data[i][emailCol])){
            ...
            member = AdminDirectory.Members.insert(
                memberRole, "testgoogle@mindsmatterseattle.org")
        }
    }
    catch(e){
...
function listAcceptedUsers() {
    ...
    for (i=2020
         i <= 2022
         i++){
        student_groups[i] = GroupsApp.getGroupByEmail(
            "students"+i+"@mindsmatterseattle.org")
        student_year_inds[i] = []
    }
    ...
    for (i=1
        ...
        if (data[i][volunteerTypeCol] == = 'Student'){
            ...
            if (data[i][yearCol] > 2019){
                ...
                Utilities.sleep(1000)
                addGroupMember(
                    email, student_groups[data[i][yearCol]].getEmail(), dry_run)
            }
        }
        if (isUser(email)){
        ...
        if (admin_accounts.indexOf(user.primaryEmail) == -1){
            if (!isUserbyName(user, rangeValues, firstNameCol, lastNameCol)){
                ...
                if (!isUserByEmail(user, rangeValues, emailCol)){
                    Logger.log(
                        "User not found by name or email in active salesforce: " + user.name.fullName)
                }
            }
        }
    }
